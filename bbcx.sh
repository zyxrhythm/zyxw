...
Enter a valid domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)<a/>.<br><br><br>For the domain, <a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> will query the Registry's whois server/s and the whois server/s of the Registrar (if found from the whois information provided by the Registrar's whois server), and will query A.R.I.N.'s whois server for IP addresses found by DIG. <br><br><a href='https://en.wikipedia.org/wiki/Dig_(command)' target='_blank'>DIG</a> will do a series of query on Google's Public DNS server (8.8.8.8) for A records asssosciated with the naked domain and the IP address associated with the MX records. <br><br><a href='https://en.wikipedia.org/wiki/Nslookup' target='_blank'>NSLOOKUP</a> is also utilized to verify some hosts/servers. <br><br>Then a report with the vital DNS and whois information is generated.</p></div>
</body>
</html>
EOTSE
exit 0;
