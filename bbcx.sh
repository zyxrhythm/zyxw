...
Enter a valid/registered <strong>naked</strong> domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)<a/>.<br><br><br>What the roaring script engine under the hood does:<br>Utilize<a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> to do a 'whois lookup' query at the known Registry's whois server/s and at the same time do the same query at the whois server/s of the Registrar (if a servers is found from the whois information provided by the Registrar's whois server). <br>--Also does the same query at A.R.I.N.'s whois server for I.P. v4 addresses found by DIG. <br><br>Then <a href='https://en.wikipedia.org/wiki/Dig_(command)' target='_blank'>DIG</a> will do a series of query to Google's Public DNS server (8.8.8.8) for A records asssosciated with the naked domain and the I.P. address (A records) associated to the hostnames found on the MX records. <br><br><a href='https://en.wikipedia.org/wiki/Nslookup' target='_blank'>NSLOOKUP</a> is also utilized to verify some hosts/servers. <br><br>Then a report with vital DNS info (about the web and mail service under the domain) and whois information is generated.</p></div>
</body>
</html>
EOTSE
exit 0;
