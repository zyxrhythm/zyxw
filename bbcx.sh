...
Enter a valid/registered <strong>naked</strong> domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>.<br><br><br>To get the whois information, the script utilizes <a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> to do a 'whois lookup' query at the known Registry's whois server for that TLD ( see the list of known servers <a href='/cgi-bin/whois.conf.sh' target='_blank'>here</a> ),and does the same query at the whois server of the Registrar (if a server is found from the whois information provided by the Registry's whois server).<br><br>
