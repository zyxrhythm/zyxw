...
Enter a valid/registered <strong>naked</strong> domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>.<br><br><br>To get the whois information, the script utilizes <a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> and does a 'whois lookup' query at the known Registry whois server for that TLD ( see the list of known servers <a href='/cgi-bin/whois.conf.sh' target='_blank'>here</a> ),and does the same query at the whois server of the Registrar ( if a server is found from the whois information provided by the Registry's whois server ).<br><br>Then <a href='https://en.wikipedia.org/wiki/Dig_(command)' target='_blank'>DIG</a> will do a series of query to one of the valid  authoritative name servers under the domain, if all name servers under the domain are invalid and cannot be queried, the script will fall back to Google's Public DNS server (8.8.8.8) for cached records. First, A records asssosciated with the naked domain are 'looked up', then A records (IP Addresses) associated to the hostnames found on the MX records. <br><br>---IP addreses found by DIG will be queried by WHOIS at A.R.I.N.'s whois servers/s to get the the Organization responsible for the IP address. <br><br>(<a href='https://en.wikipedia.org/wiki/Nslookup' target='_blank'>NSLOOKUP</a> is also utilized to verify some hosts/servers.) <br><br>Finally a report with vital whois and DNS info ( DNS record info about the web and mail service under the domain )  is generated and displayed.</p></div>
...
exit 0;
uthor will be much appreciated.
################################################
#https://github.com/zyxrhythm
#############################################
