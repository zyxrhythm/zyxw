...
do
   ...
   echo "<br> &nbsp; &nbsp; $line --- <a href='/cgi-bin/bbcws.sh?doi=$line' target='_blank' style='color:tomato' class='tooltip'> <span id='noselect'>&#9654;</span> <span class='tooltiptext' ><br>Click to get the full raw whois information from ARIN.<br><br></span></a> ${nsax##*:}";
done < <(printf '%s\n' "$nsr2");
   ...
   echo "&nbsp; &nbsp;$nsr2 --- <a href='/cgi-bin/bbcws.sh?doi=$nsr2' target='_blank' style='color:tomato' class='tooltip'> <span id='noselect'>&#9654;</span> <span class='tooltiptext' ><br>Click to get the full raw whois information of this I.P. address from ARIN.<br><br></span></a> ${nsax2#*:}"
fi
   echo '<br><br>'
done < <(printf '%s\n' "$1");
echo "--------------------------</p>
...
echo "<strong class='tooltip'><label>$mxr1a </label><span class='tooltiptext' ><br>MX priority: $mxr1a <br><br></span></strong> &nbsp;$mxr1 <br> ";
...
do
   ...
   echo "<br> &nbsp; &nbsp; $line --- <a href='/cgi-bin/bbcws.sh?doi=$line' target='_blank' style='color:tomato' class='tooltip' > <span id='noselect'>&#9654;</span> <span class='tooltiptext' >
<br>Click this to get the full raw whois information of this I.P. address from ARIN's whois server/s.<br><br>
</span></a> ${mxax#*:}";
done < <(printf '%s\n' "$mxr2");
   ...
   echo "&nbsp; &nbsp;$mxr2 --- <a href='/cgi-bin/bbcws.sh?doi=$mxr2' target='_blank' style='color:tomato' class='tooltip' > <span id='noselect'>&#9654;</span> <span class='tooltiptext' >
<br>Click this to get the full raw whois information of this I.P. address from ARIN's whois server/s.<br><br>
</span></a> ${mxax2#*:}"
   fi
fi
...
Enter a valid/registered <strong>naked</strong> domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>.<br><br><br>To get the whois information, the script utilizes <a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> to do a 'whois lookup' query at the known Registry whois server for that TLD ( see the list of known servers <a href='/cgi-bin/whois.conf.sh' target='_blank'>here</a> ),and does the same query at the whois server of the Registrar ( if a server is found from the whois information provided by the Registry's whois server ).<br><br>Then <a href='https://en.wikipedia.org/wiki/Dig_(command)' target='_blank'>DIG</a> will do a series of query to Google's Public DNS server (8.8.8.8). First, A records asssosciated with the naked domain, then A records (IP Addresses) associated to the hostnames found on the MX records. <br><br>---IP addreses found by DIG will be queried by WHOIS at A.R.I.N.'s whois servers/s, the Organization responsible for the IP address will be extracted from the whois result. <br><br>(<a href='https://en.wikipedia.org/wiki/Nslookup' target='_blank'>NSLOOKUP</a> is also utilized to verify some hosts/servers.) <br><br>Finally a report with vital whois and DNS ( records about the web and mail service under the domain ) info  is generated and displayed.</p></div>
