...
Enter a valid domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)<a/>.<br><br><br>For the domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>, <a href='https://en.wikipedia.org/wiki/WHOIS#Software' target='_blank'>WHOIS</a> will do a 'whois' lookup query at the Registry's whois server/s and at the whois server/s of the Registrar (if found from the whois information provided by the Registrar's whois server). Also does a query at A.R.I.N.'s whois server for I.P. v4 addresses found by DIG. <br><br><a href='https://en.wikipedia.org/wiki/Dig_(command)' target='_blank'>DIG</a> will do a series of query to Google's Public DNS server (8.8.8.8) for A records asssosciated with the naked domain and the I.P. address (A records) associated to the hostnames found on the MX records. <br><br><a href='https://en.wikipedia.org/wiki/Nslookup' target='_blank'>NSLOOKUP</a> is also utilized to verify some hosts/servers. <br><br>Then a report with vital DNS info (about the web and mail service under the domain) and whois information is generated.</p></div>
</body>
</html>
EOTSE
exit 0;
...
tld0=$( echo $domain | rev | cut -d "." -f1 | rev );

case $tld0 in
   $tldlist0)
if [[ $( echo "$domain" | grep -o "\." | wc -l) -gt "1" ]] && [[ -z "$domhv" ]]; then domvarx="(A <a href='https://en.wikipedia.org/wiki/Generic_top-level_domain'  target='_blank' >gTLD</a> sub domain)"; hubad0="y"; else domvarx=" " ; hubad0="y"; fi;
;;

	$tldlist1)
if [[ $( echo "${domain#*.}" | grep -o "\." | wc -l) -gt "1" ]] && [[ -z "$domhv" ]]; then domvarx="(A <a href='https://en.wikipedia.org/wiki/Country_code_top-level_domain' target='_blank' >ccTLD</a> sub domain)"; hubad0="y"; else domvarx=" " ; hubad0="y"; fi;
;;

	*)
if [[ $( echo "$domain" | grep -o "\." | wc -l) -lt "1" ]] && [[ -z "$domhv" ]]; then domvarx="(Not a domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>/ sub domain)"; hubad0="x"; else domvarx=" " ; hubad0="x"; fi;
;;
