...
domhv=$( echo "$(nslookup "$domain")" | grep -e 'NXDOMAIN'  );
	...
	echo "<body><div id='divClipboard0'><p><strong>Input</strong> : $domain - $domvarx <br> <br>Please input a valid/registered <strong>naked</strong> domain name <a href='https://en.wikipedia.org/wiki/Fully_qualified_domain_name' target='_blank'>(FQDN)</a>.<br><br><br><br>Additional info from Who You <a href='/cgi-bin/bbcws.sh?domain=$domain' target='_blank' >here.</a></p></div></body></html>"; exit 0;

elif [[ "$dvc" = "%ianawh" ]]; 
	then echo "<body><div id='divClipboard0'><p><strong>Input</strong> : $domain - is a TLD<br><br><pre>$zyx<pre></p></div></body></html>"; exit 0;
	else true; 
